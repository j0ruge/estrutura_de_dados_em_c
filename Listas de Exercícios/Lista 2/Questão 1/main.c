...
int main(int argc, char* argv[]) {
	...
	do {
		...
		printf("\n(6) Ordenar o vetor");
		printf("\n(7) Realizar busca binária (Ordenar Primeiro (6))");
		printf("\n(8) Excluir número");
		printf("\n(9) Alterar número");
		printf("\n(0) Sair");
		printf("\n\nDigite uma opcao: ");
		...
		case 8:
			...
			break;
		case 9:
			printf("\nInforme o numero a ser ALTERADO: ");
			scanf_s("%d", &elemento);
			printf("\nInforme o numero a ser INSERIDO: ");
			scanf_s("%d", &valor);
			indice = alterar_elemento(vetor_dinamico, quantidade_elementos, elemento, valor);
			if (indice != NOT_FOUND) {
				printf("A última posição que o elemento foi alterado: %d", indice);
			}
			else {
				printf("Elemento nao encontrado no vetor!");
			}
