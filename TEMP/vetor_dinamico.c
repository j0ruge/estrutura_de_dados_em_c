...
int main(int argc, char* argv[]) {
	...
	do {
		...
		case 3:
			...
			if (indice != NOT_FOUND) {
				printf("Elemento encontrado na posic�o %d", indice);
			}
			else {
...
int* incluir_valor(int* vetor_dinamico, int valor, int* quantidade_elementos, int* quantidade_total_elementos) {
	...
	{ 
		printf("\nO vetor estava vazio - primeira aloca��o!");
		vetor_dinamico = (int*)malloc(FATOR_REALLOC * sizeof(int));
		if (!vetor_dinamico) { //n�o conseguiu espa�o em mem�ria
			return NULL;
	}
		*quantidade_total_elementos = 2;
		*quantidade_elementos = 0;
	}
	if (*quantidade_elementos == *quantidade_total_elementos) { //meu vetor est� cheio
		printf("\nO vetor precisa de realoca��o!");
